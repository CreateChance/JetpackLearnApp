...
class MainActivity : AppCompatActivity() {
    ...
    // Room 不允许在 UI 线程操作 DB，因此需要异步执行
    fun add(view: View) {
        InsertStudentTask(studentDao).execute(
    ...
    fun delete(view: View) {
        DeleteStudentTask(studentDao).execute(
            Student(
                id = 1
            )
        )
    ...
    fun modify(view: View) {
        ModifyStudentTask(studentDao).execute(
            Student(
                id = 1,
                name = "SAfaksdfkaslkd"
            )
    ...
    // Room 不允许在 UI 线程操作 DB，因此需要异步执行
    fun query(view: View) {
        QueryAllStudentsTask(studentDao, recyclerAdapter).execute()
    }
}
